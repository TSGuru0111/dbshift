...
> reports `blocked by RDS-004` after the change. Every SCT action item is now
> **routed** by where the fix belongs (source / target / decision / human) and
> who may make it (automatic / AI-drafted-and-gated / human-only), from
> `sct/route.py` — a reviewed table, not the model's opinion. On `DBMIG_APP`:
> 2 source, 4 target, 2 decisions, 6 needing a person; 1 automatic, 7
> AI-draftable, 6 human-only. `sct.selftest` **191/191**, `drive_sct.js`
> **37/37**.
>
> **Worth recording: the concern about losing coverage was unfounded.** The
> objection to removing the rules engine was that the gate would go blind to
> NOARCHIVELOG, supplemental logging and missing primary keys. In fact
> `DBMIG_APP` has exactly one CRITICAL — `RDS-004`, external table — and SCT
> reports the same thing as `5200`, with **two occurrences where the rule found
> one**. `OPS-001`/`OPS-002`/`DQ-001` are already not-applicable on a full-load
> run, and CDC readiness is checked independently by the Connect preflight. See
> [`../16-sct-runner.md`](../16-sct-runner.md).
>
> Earlier — 2026-09-17 (later).** **A second path drives the real AWS
> SCT.** `sct/` runs the actual tool over its batch CLI, with a **target
> dropdown** (RDS for Oracle, RDS for PostgreSQL in scope; Aurora PostgreSQL and
> Redshift listed for comparison but carrying no migration engine) and serves
> **SCT's own PDF and CSV verbatim** — the opposite provenance claim from
> `report/export.py` below, which must keep saying "not generated by AWS SCT".
> **`assess/` is not replaced:** SCT does not look at OPS, SEC, DQ or PERF
> findings, and Phases 3, 5 and 7 all read `assessment.json` for them. Host is
> local for the demo and EC2 for a customer — **never Lambda**, for reasons kept
> in full in [`../16-sct-runner.md`](../16-sct-runner.md).
>
> **SCT 1.0.677 is installed and runs**, and the console panel is live: target
> dropdown, streaming progress, sortable action items, SCT's own PDF/CSV.
> `sct.selftest` **65/65**; browser drive `drive_sct.js` **32/32** in headless
> Edge. **A real SCT report exists**: the grant
> (`08_grant_sct_dictionary.sql`) was applied and SCT ran end to end —
> **33 occurrences → 12 action items** on `DBMIG_APP`, **75 → 14** across three
> schemas from the console, PDF and CSV **byte-identical** to SCT's own files.
> The parser reads SCT's real 17-column format with **zero unmapped columns**
> after the first run corrected four wrong guesses. `sct.selftest` **79/79**.
>
> Earlier — **2026-09-17.** **The assessment downloads as PDF and Excel**, in the shape AWS SCT reports it — action items graded by the work a person must do, what converts automatically, the DMS checks. Produced by this engine's own rules and **labelled as such on every page and every sheet**; not generated by AWS SCT. `report/export.py`, self-test 42/42.
>
> Earlier — **2026-09-16 (later).** **The issues read as a table, and the
...
The follow-up ask was for the real thing: "I dont need preset rules i want to use
directly SCT tool", with a target dropdown and SCT's own PDF and Excel.

Built as `sct/` — `targets.py`, `toolchain.py`, `scenario.py`, `parse.py`,
`runner.py`, `run.py`, `hosts/local.py`. Full detail in
[`../16-sct-runner.md`](../16-sct-runner.md); the decisions worth repeating here:

- **`assess/` stays.** SCT reports schema and stored-code conversion. It does
  **not** look at NOARCHIVELOG, supplemental logging, missing primary keys or
  grants — and Phases 3, 5 and 7 read `assessment.json` for exactly those. "Use
  SCT instead of the rules" would have broken the gate, sizing and the DMS
  preflight. The console shows both paths.
- **There is no SCT API.** It is a stateful desktop Java app with a batch CLI
  over `.scts` scenario files, so driving it means generating that script. The
  host is therefore an operational seam, not an architectural one.
- **Lambda was asked for and does not work.** Not for the expected reasons —
  container images get 10 GB and the CLI is genuinely headless — but because
  Lambda's filesystem is read-only apart from `/tmp` while SCT writes in its own
  install directory, the 15-minute ceiling is hard where a customer estate
  assesses for hours with no partial report, ECR-baking AWS's licensed binary is
  a redistribution question, and **nothing inside AWS can reach a local XE
  listener** without a VPN or exposing 1521 publicly. Recorded in full so it is
  not re-litigated.
- **Severity is derived and says so.** SCT grades by complexity (the work a
  person must do); `assess/` grades by severity (how dangerous it is). A parsed
  row carries both, with `severity_is_derived: true` — collapsing the two axes
  would misreport both.
- **The password does not outlive the run.** SCT's CLI takes it only inside the
  scenario file, so it touches disk; it is written to a gitignored per-run
  directory, deleted in a `finally`, and the record stores the **redacted**
  scenario. The self-test asserts it is absent from the record and from every
  refusal payload.
- **Out-of-scope targets are listed, not hidden** — a client asks about Aurora
  and Redshift — but they carry no `engine`, which is what stops one leaking into
  Phase 3. Asserted by test rather than intended.
- **CSV columns are read, not assumed.** SCT's header has varied by version, so
  headers are alias-matched and anything unplaced lands in `unmapped_columns`
  rather than being dropped.

Two things found by checking rather than assuming: the Corretto MSI failed with
exit 1602 because elevation was not available (the portable zip needs no admin —
worth knowing for a locked-down customer machine), and the JDBC driver was
verified by **connecting** to the live XE (21.11.0.0.0 → Oracle 21c XE,
`CONNECTED`) rather than by checking the file existed, because a present-but-wrong
driver otherwise surfaces as an unreadable Java stack trace inside a batch run.

`sct.selftest` **65/65**; browser drive `scripts/console-test/drive_sct.js`
**32/32** in headless Edge.

**Installed and driven the same day.** SCT 1.0.677 went in via `msiexec /a` (no
elevation, 354 files), and running it corrected four assumptions the
documentation had left standing: `.scts` is an ANTLR-parsed language and **not
JSON**, SCT ships and uses Corretto **17** rather than the documented 11, the
invocation is `-type scts -script` with eleven mandatory JVM flags, and inline
JSON is escaped once rather than twice. The authoritative format came from SCT's
own `GetCliScenario` command, which emits a `ReportCreationTemplate.scts` for
exactly this use case -- reading the vendor's template beat both the
documentation and guesswork.

**The bug worth carrying forward: SCT exits 0 when a command has failed.**
`AddSource` failed on a privilege, every later command then ran against an empty
tree and "succeeded", `SaveReportPDF` wrote nothing, and the process returned 0
-- so the runner reported a clean run that had produced no report. **An empty
assessment reads as an estate with nothing wrong with it**, in a document a
client circulates, which is the one claim Phase 1 forbids. Success now requires
an artefact, SCT's own logged errors are captured regardless of exit code, and
`exit_code_said_ok` is recorded separately so the disagreement stays visible.
...
`SELECT_CATALOG_ROLE`, which SCT does not accept as equivalent. That is the
second time this distinction has cost this project time, after row access in
`05_grant_collector_read.sql`. Script `08_grant_sct_dictionary.sql` is written
and **not run**: it reaches SYS tables including `USER$` and `LINK$`, which
belongs in a DBA's change request rather than a script this project runs on its
own initiative.

**The console panel** sits below the 50-rule result, separated by a rule,
because the two are different claims and the layout should say so. Out-of-scope
targets are marked in the dropdown and warn when selected. SCT's own log is
filtered to command boundaries -- it prints its entire settings block at
startup, which would otherwise bury them.

One test bug found while driving the browser: the provenance assertions read
`innerText`, which skips hidden subtrees, so they passed or failed on whether a
phase had run rather than on what the markup says. They read `textContent` now.

**The first real run then corrected four wrong guesses in the parser** — SCT
reported 9 of its 17 columns as not understood. `Action item` is the issue
*code*, not a title; `Occurrence` is one object's tree path, not a count (SCT
emits one row per occurrence, so rows are now grouped and counted); there is no
object-name column at all, the object being the leaf of that path; and `Group`
is the readable statement while `Category` is a slug. Plus two filesystem
surprises: the CSVs are nested under `report/ORACLE/<target>/`, and SCT writes
three of them where only one holds the action items — so the detail file is
listed first rather than letting a summary be parsed as the whole assessment.
The fixture is now SCT's real header and rows, with an assertion that no column
goes unmapped. Results on `DBMIG_APP`: **33 occurrences → 12 action items**,
1 decision / 6 complex / 5 simple.

Still to do: `hosts/ec2.py` with a customer-reviewable template plus a
`killswitch/` entry for `dbshift-sct-runner`, and a dedicated run against
`DBMIG_TELCO` alone (the console run covered it alongside two other schemas).

**2026-09-17 — the assessment downloads as PDF and Excel.** Client feedback:
"I need real SCT PDF and Excel in assess phase to be downloaded." `report/export.py`
renders the Phase 10 record — the SCT-shaped one — as the two formats people
actually circulate.

**The honesty problem was the design problem.** The numbers come from this
project's own 50-rule engine, not from running AWS SCT. The *shape* is SCT's:
action items graded by the work a person must do, the same four complexity
buckets, the same "what converts automatically" headline. That shape is why a
client can read it at all, and claiming it *is* SCT output would be the easiest
lie in this project to tell and the worst to be caught in. So one constant,
`export.PROVENANCE`, appears on the PDF's first page, in its footer on every
page, on every sheet of the workbook, and on the Assess screen itself — and the
self-test asserts all four rather than trusting them to survive a tidy-up.

- **Excel, four sheets** in the order the questions get asked: Summary, Action
  items, Findings, Stored code. Every sheet is filterable with a frozen header,
  because a client works the Action items sheet rather than reading it.
- **PDF, paginated and landscape.** Not a print of the HTML: a PDF cannot be
  expanded, so the action items are a real table with the complexity meaning
  spelled out rather than an accordion nobody can click.
- Both carry the **not-applicable findings with their reason and original
  severity**, never dropped — the same rule the CSV follows, for the same reason.
- The findings sheet reads from **Phase 2's record**, not the report record,
  which carries only rolled-up counts. Without an assessment the sheet says the
  detail is unavailable rather than appearing empty and reading as a clean estate.

Two bugs the first version had, both caught by checking the output rather than
the code: the findings sheet was empty (it looked for `rec["findings"]`, which
the report record does not carry) and the stored-code sheet had blank object
names (`code_objects` is a rollup *by type*, not one row per object).

`report.selftest_export` **42/42**; browser drive `drive_counts_export.js` now
**32/32**, downloading both files for real and checking the magic bytes.
