...
def stack_name_for(estate: str, engine: str | None = None) -> str:
    """One stack per estate *and engine*.

    The two paths can be provisioned for the same estate -- a client may want to
    see both -- and without the engine in the name the second deploy would
    silently UPDATE the first, replacing an Oracle instance with a PostgreSQL
    one. The Oracle name is left unsuffixed so the stack already deployed keeps
...
def _tags(stack: str, estate: str, run_id: str, expires: str) -> list[dict]:
    # policy.as_tag_list puts the account's required tags first -- Purpose=DMA --
    # so nothing this phase creates can be missing them.
    return policy.as_tag_list({
        "project": "dbshift",
        "estate": estate,
        "collector_run_id": run_id,
        "expires-at": expires,
        "created-by": f"dbshift-provision/{stack}",
    })
...
def render(records: dict, facts: dict, *, engine_version: str, stack_name: str, estate: str,
    ...
    # ---- engine, edition and licence ----------------------------------------
    # Phase 3 decides which of the two paths this is. On PostgreSQL there is no
    ...
    if pg:
        ...
        trace("EngineVersion", engine_version, "preflight: latest orderable in region",
              f"PostgreSQL {engine_version}; Phase 4b compiles against major "
              f"{policy.PG_TARGET_MAJOR} locally, so what was proven is what runs")
    else:
    ...
    # Option groups are an Oracle concept. PostgreSQL has none, and its data does
