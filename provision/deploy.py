...
def deploy(session, *, confirm_account: str, accept_hourly: float | None, halt_reason: str | None = None,
    ...
    # cloud_formation_template_name (formerly stack_name_free) is a WARN for rendering, but a deploy onto an existing stack
    # would only fail at CloudFormation, after the request had been recorded.
    taken = next((c for c in plan["checks"] if c["name"] in ("cloud_formation_template_name", "stack_name_free") and c["status"] != "pass"), None)
    if taken:
