...
def aws_checks(session, *, engine: str, licence: str, instance_class: str, storage_type: str,
    ...
    try:
        ...
        for n in ("engine_version", "network", "quota", "cloud_formation_template_name", "budget"):
    ...
    # cloud formation template name not already taken
    try:
        cfn = session.client("cloudformation", region_name=region)
        cfn.describe_stacks(StackName=stack_name)
        checks.append(_c("cloud_formation_template_name", WARN, f"{stack_name} already exists",
                         "A deploy would update it. Run the kill switch first for a clean target."))
    except Exception as exc:  # noqa: BLE001
        if "does not exist" in str(exc):
            checks.append(_c("cloud_formation_template_name", PASS, f"{stack_name} does not exist yet"))
        else:
            checks.append(_c("cloud_formation_template_name", BLOCKED, str(exc).splitlines()[0]))
