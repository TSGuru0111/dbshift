...
<p class="lead">{_e(r['estate'] or 'estate')} &rarr; {_e(_target_line(r))}</p>
...
<div class="tiles">
{_tile("Edition", f"{_e(td.get('edition') or 'none')}<small>{_e(td.get('licence_model') or '')}</small>",
       f"forced by {_e(forced)}" if td.get('edition') else "PostgreSQL has no editions")}
...
{_tile("Processor licences", str(td.get('processor_licences') or 'none'),
       "BYOL, held by the client" if td.get('processor_licences') else "the engine is open source")}
</div>""")
...
def _target_line(r: dict) -> str:
    """What this estate is migrating to, in the report's first sentence.

    Read from the record rather than assumed: a report that names the wrong
    target is worse than one that names none, because every figure below it is
    then read against the wrong engine.
    """
    if r.get("target_engine") == "POSTGRESQL":
        return ("Amazon RDS for PostgreSQL, a heterogeneous migration: the stored code is "
                "rewritten and the table DDL is generated, not carried across.")
    return "Amazon RDS for Oracle, with the stored code also assessed for PostgreSQL."
