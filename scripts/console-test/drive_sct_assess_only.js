// Drives Assess -> Remediate -> Blocker gate against whatever session is
// ALREADY connected and discovered, deliberately skipping Connect and
// Discover -- see the note above the "Deliberately no Connect" block below
// for why. Forked from drive_sct_1to5.js.
//
...
const OUT = path.join(__dirname, 'shots-sct-assess-only');
...
(async () => {
  ...
  // ---- Deliberately no Connect, no Discover ------------------------------
  // Every prior run of this drive re-clicked #btnConnect and #btnDiscover,
  // and Discover mints a fresh collector_run_id each time even against the
  // identical source -- that is what caused this run's own problem: Sizing
  // re-ran against a newer Discover pass than Assess/Remediate/Gate had used,
  // so records_consistent failed one way, then re-running Sizing again just
  // flipped which records were "the stale ones" without ever converging.
  // The fix is to hold Discover still and bring Assess/Remediate/Gate onto
  // whatever collector_run_id is already live, not to keep re-discovering.
  const live = await (await page.request.get(BASE + '/api/state')).json();
  log('a session is already connected', !!live.connected, live.dsn || 'not connected');
  log('a run is already live', !!live.run_id, live.run_id || 'none');
  if (!live.connected || !live.run_id) {
    console.log('No live connected session with a discovery run -- refusing to Discover '
      + 'again, since that is what caused the drift. Connect and Discover once by hand, '
      + 'then re-run this.');
    await browser.close();
    process.exit(1);
  }
  await page.goto(BASE, { waitUntil: 'networkidle' });  // pick up existing state
  ...
  // The unmodified drive never checks this, so it silently took whatever
  // cached result the server held -- on 2026-09-22 that was a 5-day-old
  // assessment of a different schema (DBMIG_APP) on localhost, while this
  // run is connected to the EC2 source as DBMIG_TELCO. Force a real one.
  const forceBox = page.locator('#sctForce');
  if (await forceBox.count() && !(await forceBox.isChecked())) await forceBox.check();
