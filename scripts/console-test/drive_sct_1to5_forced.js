...
(async () => {
  ...
  // ---- Phase 1a: Connect --------------------------------------------------
  // #dsn defaults to localhost:1521/XEPDB1 in the page's own HTML -- filling
  // only #password and clicking Connect silently reconnects to that default
  // instead of the real EC2 source. That happened once already on
  // 2026-09-22: a run left the console pointed at localhost with no schema
  // selected, and Assess then quietly served a 5-day-old cached result for
  // the wrong schema (DBMIG_APP) instead of reaching DBMIG_TELCO on EC2.
  const dsn = process.env.DBSHIFT_SOURCE_DSN || '';
  const schemaName = process.env.DBSHIFT_SOURCE_SCHEMA || '';
  if (dsn) await page.fill('#dsn', dsn);
  if (schemaName) await page.fill('#schema', schemaName);
